...
namespace CheapLoansProjectPart4._5
{
    ...
    /// Client for interacting with the GetLoanByCustomer() method in the LoanService class.
    /// </summary>
    public partial class LoansByCustomer : Page
    {
        ...
        protected void ViewData_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                // Attempts to retrieves data in the GridView using the name of a customer
                string loansJson = loanService.GetLoanByCustomer(CustName.Text.Trim());

                // Determines what should be displayed
                if (loansJson.Contains("No customers were found"))
                {
